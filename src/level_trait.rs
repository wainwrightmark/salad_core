...
pub trait LevelTrait<const GRID_SIZE: usize>: Clone {
    ...
    // fn svg_tile_position(tile: GridTile, from_centre: bool) -> Vec2 {
    //     const SCALE: f32 = 100.0;
    //     const RECT_SIZE: f32 = 90.0;
    ...
    //     let x = (pos.x * SCALE) + ((SCALE - RECT_SIZE) * 0.5);
    //     let y = (pos.y * SCALE) + ((SCALE - RECT_SIZE) * 0.5);
    ...
    //     special_colors: &Option<Vec<bevy_color::Srgba>>,
    //     special_tile_fill: Option<(GridSet, &'static str)>,
    // ) -> String {
    //     const SCALE: f32 = 100.0;
    //     const FONT_SIZE: f32 = 50.0;
    //     const RECT_SIZE: f32 = 90.0;
    //     const PATH_STROKE_WIDTH: f32 = 50.0;
    ...
    //     writeln!(&mut svg, r#"<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 {size} {size}" font-size="{FONT_SIZE}" font-family="{font_family}" font-weight="bold" style="text-align: center; text-anchor: middle;" >"# ).unwrap();

    //     let fill_color = ColorScheme::CLASSIC.tile_fill.to_hex();

    //     for (tile, _rune) in self.grid().enumerate() {
    //         if unneeded_tiles.get_bit(&tile) {
    //             continue;
    //         }

    //         let fc: &str;
    //         if let Some((tiles, ref special_color)) = special_tile_fill {
    //             if tiles.get_bit(&tile) {
    //                 fc = &special_color;
    //             } else {
    //                 fc = &fill_color
    //             }
    ...
    //         let Vec2 { x, y } = Self::svg_tile_position(tile, false);
    //         writeln!(&mut svg, r##"<rect width="{RECT_SIZE}" height="{RECT_SIZE}" x="{x}" y="{y}" fill="{fc}"> </rect>"##, ).unwrap();
    //     }

    //     let mut segment_index = 0usize;
    //     let mut last_relative_point = None;

    //     for (from, to) in solution.iter().copied().tuple_windows() {
    //         let Vec2 { x: x1, y: y1 } = Self::svg_tile_position(from, true);
    //         let Vec2 { x: x2, y: y2 } = Self::svg_tile_position(to, true);

    //         let color = ColorScheme::CLASSIC
    //             .wordline_index_to_color(segment_index, special_colors)
    //             .to_srgba()
    //             .to_hex();

    //         writeln!(&mut svg,
    //             r##"<line x1="{x1}" y1="{y1}" x2="{x2}" y2="{y2}" stroke="{color}" stroke-linecap="round" stroke-width="{PATH_STROKE_WIDTH}"  />"##

    //         ).unwrap();

    //         let relative_point =
    //             Some(Self::svg_tile_position(to, true) - Self::svg_tile_position(from, true));
    ...
    //     for (tile, rune) in self.grid().enumerate() {
    //         if unneeded_tiles.get_bit(&tile) {
    //             continue;
    //         }

    //         let Vec2 { x, y } = Self::svg_tile_position(tile, false);
    //         let text_color = if solution.contains(&tile) {
    //             ColorScheme::CLASSIC.grid_letter_selected.to_hex()
    //         } else {
    //             ColorScheme::CLASSIC.grid_letter_unselected.to_hex()
    //         };
    //         writeln!(
    //             &mut svg,
    //             r##"<text x="{}" y="{}" dominant-baseline="central" fill="{text_color}">{} </text>"##,
    //             x + (RECT_SIZE * 0.5),
    //             y + (RECT_SIZE * 0.5),
    //             rune.to_tile_string()
    //         )
